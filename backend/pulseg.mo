// PULSEG Governance Token
// ICRC-1/ICRC-2 compliant governance token for ICP Pulse platform
// Minting controlled by staking canister
...
persistent actor class PULSEG() = this {
  ...
  var init : {
    ...
    token_name = "TruePulse Governance";
    token_symbol = "PULSEG";
    decimals = 8;
    transfer_fee = 10_000; // 0.0001 PULSEG
  };

  var logo : Text = "";
  var created : Bool = false;

  // Authorized minters (staking canister can mint)
  var authorizedMinters : [Principal] = [];

  // Token initialized flag
  public query func token_created() : async Bool {
    created;
  };

  // Initialize token (one-time setup)
  public shared ({ caller }) func initialize_token(
    initial_supply : Nat,
    token_logo : Text,
    ...
    // Authorize staking canister to mint
    authorizedMinters := [staking_canister];

    // Initialize the genesis chain
    log := makeGenesisChain();
    created := true;

    #Ok("PULSEG token initialized successfully");
  };

  // Add authorized minter (only minting account owner)
  public shared ({ caller }) func add_authorized_minter(minter : Principal) : async Result<Text, Text> {
    if (not Principal.equal(caller, init.minting_account.owner)) {
      return #Err("Only minting account owner can add authorized minters");
    };

    authorizedMinters := Array.append(authorizedMinters, [minter]);
    #Ok("Authorized minter added");
  };

  // Remove authorized minter
  public shared ({ caller }) func remove_authorized_minter(minter : Principal) : async Result<Text, Text> {
    if (not Principal.equal(caller, init.minting_account.owner)) {
      return #Err("Only minting account owner can remove authorized minters");
    };

    authorizedMinters := Array.filter<Principal>(authorizedMinters, func(m) { not Principal.equal(m, minter) });
    #Ok("Authorized minter removed");
  };

  // Check if principal is authorized minter
  private func isAuthorizedMinter(principal : Principal) : Bool {
    if (Principal.equal(principal, init.minting_account.owner)) {
      return true;
    };

    for (minter in authorizedMinters.vals()) {
      if (Principal.equal(principal, minter)) {
        return true;
      };
    };
    false
  ...
  public shared ({ caller }) func mint(
    ...
    if (not isAuthorizedMinter(caller)) {
      return #Err(#GenericError({
        error_code = 403;
        message = "Caller is not authorized to mint tokens"
      }));
    };
  ...
  // ICRC-1/ICRC-2 Standard Implementation
  ...
  // Query authorized minters
  public query func get_authorized_minters() : async [Principal] {
    authorizedMinters
  };
};
